"""Slugs de départements pour construire les URLs des sources scrappées (paruvendu…).

Mapping statique (offline) code INSEE département -> slug nom (sans accents), validé
contre le format d'URL de Paruvendu (`/immobilier/vente/maison/{slug}-{code}/`).
"""
...
DEPT_SLUG = {
    ...
    "18": "cher", "19": "correze", "21": "cote-d-or", "22": "cotes-d-armor", "23": "creuse",
    "24": "dordogne", "25": "doubs", "26": "drome", "27": "eure", "28": "eure-et-loir",
    "29": "finistere", "2A": "corse-du-sud", "2B": "haute-corse", "30": "gard",
...
def dept_slug(code: str | None) -> str | None:
    """Renvoie 'ardeche-07' pour '07', ou None si inconnu."""
    if not code:
        return None
    s = DEPT_SLUG.get(str(code).upper())
    return f"{s}-{str(code).upper()}" if s else None
